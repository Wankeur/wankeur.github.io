// Admin panel functionality with local/live detection
...
function checkServerType() {
    ...
    // Check if it's localhost or local IP with port 3001
    isLocalServer = (hostname === 'localhost' || hostname === '127.0.0.1' || hostname.startsWith('192.168.')) && port === '3001';
    
    return isLocalServer;
}

// Show appropriate content based on server type
function showAppropriateContent() {
    const loginSection = document.getElementById('login-section');
    const adminPanel = document.getElementById('admin-panel');
    const localOnlyMessage = document.getElementById('local-only-message');
    
    if (isLocalServer) {
        // Running locally - show normal admin functionality
        if (authToken) {
            showAdminPanel();
            loadProjects();
        } else {
        ...
        if (localOnlyMessage) localOnlyMessage.style.display = 'none';
    } else {
        // Running on live server - show message
        if (loginSection) loginSection.style.display = 'none';
        if (adminPanel) adminPanel.style.display = 'none';
        if (localOnlyMessage) localOnlyMessage.style.display = 'block';
    }
}

// Check authentication and server type on page load
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
    ...
    if (!isLocalServer) {
        alert('Admin features are only available when running the site locally.');
        return;
    }
