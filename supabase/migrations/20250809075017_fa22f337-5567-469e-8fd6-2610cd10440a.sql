-- Create trigger to auto-create profiles on new auth users (if missing)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Backfill profiles for existing users that don't have a profile yet
...
-- Allow everyone to view the names of users who commented on approved projects
CREATE POLICY IF NOT EXISTS "Public can view profiles of comment authors on approved projects"
ON public.profiles
FOR SELECT
USING (
  EXISTS (
